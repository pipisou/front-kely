...
export class AjoutServiceDetailsComponent implements OnInit {
  ...
  ngOnInit(): void {
    ...
    this.loadServices();
    this.loadCategories();
    this.loadServicePrerequis();  // Charger les services prérequis
  }
  ...
  onSubmit() {
    ...
    // Si un servicePrerequis est sélectionné, vérifier si ses categorieDeVehicule.id correspond à celui du formulaire
    if (formData.servicePrerequis) {
